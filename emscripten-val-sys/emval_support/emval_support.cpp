...
EMSCRIPTEN_BINDINGS(MyBindings) {
    class_<std::function<val()>>("ListenerCallback0")
        .constructor<>()
        .function("handleEvent", &std::function<val()>::operator());
    class_<std::function<val(val)>>("ListenerCallback1")
        .constructor<>()
        .function("handleEvent", &std::function<val(val)>::operator());
    class_<std::function<val(val, val)>>("ListenerCallback2")
        .constructor<>()
        .function("handleEvent", &std::function<val(val, val)>::operator());
    class_<std::function<val(val, val, val)>>("ListenerCallback3")
        .constructor<>()
        .function(
            "handleEvent", &std::function<val(val, val, val)>::operator()
        );
    class_<std::function<val(val, val, val, val)>>("ListenerCallback4")
        .constructor<>()
        .function(
            "handleEvent", &std::function<val(val, val, val, val)>::operator()
        );
};
...
void _emval_add_event_listener(EM_VAL em, const char *name, void *data) {
    ...
    auto func = val(std::function<val(val)>([=](val ev) -> val {
...
EM_VAL _emval_take_fn(unsigned char argcount, void *data) {
    ...
    switch (argcount) {
    case 0:
        func = val(std::function<val()>([=]() -> val {
            return val::take_ownership(emscripten_val_rust_caller0(data));
        }));
        break;
    case 1:
        func = val(std::function<val(val)>([=](val ev) -> val {
            return val::take_ownership(
                emscripten_val_rust_caller1(ev.as_handle(), data)
            );
        }));
        break;
    case 2:
        func = val(std::function<val(val, val)>([=](val a0, val a1) -> val {
        ...
        break;
    case 3:
        func = val(std::function<val(val, val, val)>(
        ...
        break;
    case 4:
        func = val(std::function<val(val, val, val, val)>(
